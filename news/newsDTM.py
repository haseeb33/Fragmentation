...
with open("news.csv", "r") as f:
    ...
    for row in csv_reader:
        if row[1] == "2016" or row[1] == "2017":
            ...
            if int(row[2]) < 10:
                date.append(str(row[1]) + "-0" + str(row[2]))
            else:
                date.append(str(row[1]) + "-" + str(row[2]))
            

sorted_years = sorted(enumerate(date), key=lambda x:x[1])

ts = []; ts_val = []; documents = []; prev = '2016-01' ; doc_count = 0 ; 
count = 0 ; max_doc_size = 171030
for pos in range(len(sorted_years)-1):
...
writer = pd.ExcelWriter("news90topics_DTM.xlsx")
